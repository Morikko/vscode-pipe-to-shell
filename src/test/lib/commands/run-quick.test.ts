...
describe("RunQuickCommand", () => {
  describe('When command is specified and "processEntireTextIfNoneSelected" is set to "false"', () => {
    ...
    when(
      workspaceAdapter.getConfig("editWithShell.favoriteCommands"),
    ).thenReturn([{ id: "favourite-command-1", command: "COMMAND_STRING" }]);
