...
export class RunQuickCommand extends RunCommand {
  ...
  protected getCommandText(): Promise<string | undefined> {
    const commandId = this.workspace.getConfig<string>(
      `${EXTENSION_NAME}.quickCommand${this.commandNumber}`,
    );
    const favoriteCommands = this.workspace.getConfig<FavoriteCommand[]>(
      `${EXTENSION_NAME}.favoriteCommands`,
    );
